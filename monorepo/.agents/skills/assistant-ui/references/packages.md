...
- Pre-built components are added via project templates in: `@/components/assistant-ui/thread` and
  `@/components/assistant-ui/thread-list`
...
- Node.js >=24 is only required to build the assistant-ui monorepo itself; consuming apps follow their framework's Node
  requirement
